...
<div style="display:flex; gap:10px;">
    <form method="POST" action="{{ route('doctor.availability.update') }}">
        ...
        <button type="submit" style="padding:10px 20px; border-radius:8px; border:2px solid {{ $doctor->availability_status=='Available' ? '#22c55e' : '#e5e7eb' }}; background:{{ $doctor->availability_status=='Available' ? '#f0fdf4' : '#fff' }}; color:{{ $doctor->availability_status=='Available' ? '#166534' : '#6b7280' }}; font-weight:600; font-size:13px; cursor:pointer;">
            ✅ Available
        </button>
    </form>
    <form method="POST" action="{{ route('doctor.availability.update') }}">
        ...
        <button type="submit" style="padding:10px 20px; border-radius:8px; border:2px solid {{ $doctor->availability_status=='Not Available' ? '#ef4444' : '#e5e7eb' }}; background:{{ $doctor->availability_status=='Not Available' ? '#fef2f2' : '#fff' }}; color:{{ $doctor->availability_status=='Not Available' ? '#b91c1c' : '#6b7280' }}; font-weight:600; font-size:13px; cursor:pointer;">
            ❌ Not Available
        </button>
    </form>
</div>
